...
func GetMovie(Id string) (movie *Movie) {
	...
	if err := cacheStore.Get(key, &movie); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", fmt.Sprintf("Failed getting Trakt movie (%s), check your logs.", Id), config.AddonIcon())
		}
...
func Userlists() (lists []*List) {
	...
	if traktUsername == "" {
		xbmc.Notify("Torrents", "LOCALIZE[30149]", config.AddonIcon())
		return lists
	}
	...
	if resp.Status() != 200 {
		...
		xbmc.Notify("Torrents", errMsg, config.AddonIcon())
		log.Warningf(errMsg)
		return lists
	}
