...
func GetShow(Id string) (show *Show) {
	...
	if err := cacheStore.Get(key, &show); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", fmt.Sprintf("Failed getting Trakt show (%s), check your logs.", Id), config.AddonIcon())
			return
		}
...
func GetShowByTMDB(tmdbId string) (show *Show) {
	...
	if err := cacheStore.Get(key, &show); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed getting Trakt show using TMDB ID, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&show); err != nil {
			log.Warning(err)
...
func GetShowByTVDB(tvdbId string) (show *Show) {
	...
	if err := cacheStore.Get(key, &show); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed getting Trakt show using TVDB ID, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&show); err != nil {
			log.Warning(err)
...
func GetEpisode(id string) (episode *Episode) {
	...
	if err := cacheStore.Get(key, &episode); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed getting Trakt episode, check your logs.", config.AddonIcon())
			return
		}
...
func GetEpisodeByTMDB(tmdbId string) (episode *Episode) {
	...
	if err := cacheStore.Get(key, &episode); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed getting Trakt episode using TMDB ID, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&episode); err != nil {
			log.Warning(err)
...
func GetEpisodeByTVDB(tvdbId string) (episode *Episode) {
	...
	if err := cacheStore.Get(key, &episode); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed getting Trakt episode using TVDB ID, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&episode); err != nil {
			log.Warning(err)
