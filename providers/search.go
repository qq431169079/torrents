...
func processLinks(torrentsChan chan *bittorrent.Torrent, sortType int) []*bittorrent.Torrent {
	...
	dialogProgressBG := xbmc.NewDialogProgressBG("Torrents", "LOCALIZE[30117]", "LOCALIZE[30117]", "LOCALIZE[30118]")
	go func() {
		for {
			select {
			...
			case msg, ok := <-progressUpdate:
				...
				if dialogProgressBG != nil {
					if msg != "skip" {
						dialogProgressBG.Update(progress * 100 / progressTotal, "Torrents", msg)
					}
				} else {
					return
				}
	...
	wg.Wait()
	dialogProgressBG.Update(100, "Torrents", "LOCALIZE[30117]")
	...
	dialogProgressBG.Update(progress * 100 / progressTotal, "Torrents", progressMsg)
	...
	go func() {
		...
		dialogProgressBG.Update(100, "Torrents", progressMsg)
