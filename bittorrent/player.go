...
import (
	...
	"github.com/videofree/torrents/broadcast"
	"github.com/videofree/torrents/diskusage"
...
func (btp *BTPlayer) addTorrent() error {
	...
	if btp.bts.config.DownloadPath == "." {
		xbmc.Notify("Torrents", "LOCALIZE[30113]", config.AddonIcon())
...
func (btp *BTPlayer) Buffer() error {
	...
	btp.dialogProgress = xbmc.NewDialogProgress("Torrents", "", "", "")
...
func (btp *BTPlayer) CheckAvailableSpace() bool {
	if btp.diskStatus != nil {
		...
		if availableSpace < sizeLeft {
			...
			xbmc.Notify("Torrents", "LOCALIZE[30207]", config.AddonIcon())
...
func (btp *BTPlayer) chooseFile() (int, error) {
	...
	for i := 0; i < numFiles; i++ {
		...
		if re.MatchString(fileName) && size > 10 * 1024 * 1024 {
			...
			if !xbmc.DialogConfirm("Torrents", "LOCALIZE[30303]") {
				btp.notEnoughSpace = true
...
func (btp *BTPlayer) Close() {
	...
	if btp.askToKeepDownloading == true {
		if !xbmc.DialogConfirm("Torrents", "LOCALIZE[30146]") {
	...
	if btp.askToDelete == true && (btp.askToKeepDownloading == false || askedToKeepDownloading == false) {
		if xbmc.DialogConfirm("Torrents", "LOCALIZE[30269]") {
			askedToDelete = true
		}
	}
			...
			if int(status.GetState()) == 1 || btp.isRarArchive {
				...
				if btp.isRarArchive && progress >= 1 {
					...
					if err != nil {
						btp.log.Error(err)
						btp.bufferEvents.Broadcast(err)
						xbmc.Notify("Torrents", "LOCALIZE[30304]", config.AddonIcon())
						return
					}
					...
					if err != nil {
						btp.log.Error(err)
						btp.bufferEvents.Broadcast(err)
						xbmc.Notify("Torrents", "LOCALIZE[30305]", config.AddonIcon())
						return
					}
					...
					if err != nil {
						btp.log.Error(err)
						btp.bufferEvents.Broadcast(err)
						xbmc.Notify("Torrents", "LOCALIZE[30306]", config.AddonIcon())
						return
					}
...
func (btp *BTPlayer) findExtracted(destPath string) {
	...
	if err != nil {
		btp.log.Error(err)
		btp.bufferEvents.Broadcast(err)
		xbmc.Notify("Torrents", "LOCALIZE[30307]", config.AddonIcon())
		return
	}
	if len(files) == 1 {
