...
func tmdbCheck(key string) bool {
	...
	if err != nil {
		log.Error(err.Error())
		xbmc.Notify("Torrents", "TMDB check failed, check your logs.", config.AddonIcon())
		return false
	...
	for i := 0; i < PagesAtOnce; i++ {
      ...
      urlValues := tmpParams.AsUrlValues()
			rateLimiter.Call(func() {
				...
				if err != nil {
					log.Error(err.Error())
					xbmc.Notify("Torrents", "Failed listing entities, check your logs.", config.AddonIcon())
				} else if resp.Status() != 200 {
...
func Find(externalId string, externalSource string) *FindResult {
	...
	if err := cacheStore.Get(key, &result); err != nil {
      ...
      urlValues := napping.Params{
			...
			if err != nil {
				log.Error(err.Error())
				xbmc.Notify("Torrents", "Failed Find call, check your logs.", config.AddonIcon())
			} else if resp.Status() != 200 {
