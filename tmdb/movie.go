...
func GetImages(movieId int) *Images {
	...
	if err := cacheStore.Get(key, &images); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				log.Error(err)
				xbmc.Notify("Torrents", fmt.Sprintf("Failed getting images for movie %d, check your logs.", movieId), config.AddonIcon())
...
func GetMovieById(movieId string, language string) *Movie {
	...
	if err := cacheStore.Get(key, &movie); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				log.Error(err)
				xbmc.Notify("Torrents", fmt.Sprintf("Failed getting movie %s, check your logs.", movieId), config.AddonIcon())
			} else if resp.Status() == 429 {
...
func GetMovieGenres(language string) []*Genre {
	...
	if err := cacheStore.Get(key, &genres); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				log.Error(err)
				xbmc.Notify("Torrents", "Failed getting movie genres, check your logs.", config.AddonIcon())
			} else if resp.Status() == 429 {
...
func SearchMovies(query string, language string, page int) (Movies, int) {
	...
	rateLimiter.Call(func() {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed searching movies, check your logs.", config.AddonIcon())
		} else if resp.Status() == 429 {
...
func GetIMDBList(listId string, language string, page int) (movies Movies, totalResults int) {
	...
	if err := cacheStore.Get(key, &movies); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				log.Error(err)
				xbmc.Notify("Torrents", "Failed getting IMDb list, check your logs.", config.AddonIcon())
			} else if resp.Status() == 429 {
		...
		for p := 0; p < PagesAtOnce; p++ {
			...
			go func(p int) {
				...
				rateLimiter.Call(func() {
					...
					if err != nil {
						log.Error(err)
						xbmc.Notify("Torrents", "Failed while listing movies, check your logs.", config.AddonIcon())
					} else if resp.Status() == 429 {
