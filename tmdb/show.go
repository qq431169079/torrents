...
func GetShowImages(showId int) *Images {
	...
	if err := cacheStore.Get(key, &images); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				log.Error(err)
				xbmc.Notify("Torrents", "Failed getting images, check your logs.", config.AddonIcon())
	...
	if err := cacheStore.Get(key, &show); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				...
				LogError(err)
				xbmc.Notify("Torrents", "Failed getting show, check your logs.", config.AddonIcon())
			} else if resp.Status() == 429 {
...
func SearchShows(query string, language string, page int) (Shows, int) {
	...
	rateLimiter.Call(func() {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("Torrents", "Failed searching shows check your logs.", config.AddonIcon())
		} else if resp.Status() == 429 {
		...
		for p := 0; p < PagesAtOnce; p++ {
			...
			go func(p int) {
				...
				rateLimiter.Call(func() {
					...
					if err != nil {
						log.Error(err)
						xbmc.Notify("Torrents", "Failed while listing shows, check your logs.", config.AddonIcon())
					} else if resp.Status() == 429 {
...
func GetTVGenres(language string) []*Genre {
	...
	if err := cacheStore.Get(key, &genres); err != nil {
		rateLimiter.Call(func() {
			...
			if err != nil {
				log.Error(err)
				xbmc.Notify("Torrents", "Failed getting TV genres, check your logs.", config.AddonIcon())
			} else if resp.Status() == 429 {
