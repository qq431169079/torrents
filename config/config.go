...
func Reload() *Configuration {
	...
	if downloadPath == "." {
		xbmc.Dialog("Torrents", "LOCALIZE[30113]")
		xbmc.AddonSettings("plugin.video.torrents")
		go waitSettingsSet()
	} else if err := IsWritablePath(downloadPath); err != nil {
		log.Error(err)
		xbmc.Dialog("Torrents", err.Error())
		xbmc.AddonSettings("plugin.video.torrents")
	} else {
		settingsSet = true
	}
...
func CheckBurst() {
	// Check for enabled providers and Torrents Burst
	hasBurst := false
	...
	for _, addon := range xbmc.GetAddons("xbmc.python.script", "executable", "all", []string{"name", "version", "enabled"}).Addons {
		if strings.HasPrefix(addon.ID, "script.torrents.") {
			if addon.ID == "script.torrents.burst" && addon.Enabled == true {
				hasBurst = true
			}
	...
	if !hasBurst {
		...
		if xbmc.DialogConfirm("Torrents", "LOCALIZE[30271]") {
			xbmc.PlayURL("plugin://script.torrents.burst/")
			for _, addon := range xbmc.GetAddons("xbmc.python.script", "executable", "all", []string{"name", "version", "enabled"}).Addons {
				if addon.ID == "script.torrents.burst" && addon.Enabled == true {
					hasBurst = true
				}
			...
			if hasBurst {
				...
				xbmc.Notify("Torrents", "LOCALIZE[30272]", AddonIcon())
			} else {
				xbmc.Dialog("Torrents", "LOCALIZE[30273]")
			}
		}
	}
}
