...
import (
	...
	"github.com/videofree/torrents/trakt"
...
func syncMoviesList(listId string, updating bool) (err error) {
	...
	if !updating {
		...
		if xbmc.DialogConfirm("Torrents", fmt.Sprintf("LOCALIZE[30277];;%s", label)) {
...
func removeMovie(ctx *gin.Context, tmdbId string) error {
	...
	if ctx != nil {
		if xbmc.DialogConfirm("Torrents", fmt.Sprintf("LOCALIZE[30278];;%s", movieName)) {
...
func removeShow(ctx *gin.Context, tmdbId string) error {
	...
	if ctx != nil {
		if xbmc.DialogConfirm("Torrents", fmt.Sprintf("LOCALIZE[30278];;%s", show.Name)) {
			xbmc.VideoLibraryClean()
		} else {
			clearPageCache(ctx)
		}
...
func removeEpisode(tmdbId string, showId string, scraperId string, seasonNumber int, episodeNumber int) error {
	...
	if !alreadyRemoved {
		...
		return errors.New("Nothing left to remove from Torrents")
	}
...
func AddMovie(ctx *gin.Context) {
	...
	if movie, err := isDuplicateMovie(tmdbId); err != nil {
		...
		xbmc.Notify("Torrents", fmt.Sprintf("LOCALIZE[30287];;%s", movie.Title), config.AddonIcon())
		return
	}
	...
	if xbmc.DialogConfirm("Torrents", fmt.Sprintf("LOCALIZE[30277];;%s", movie.Title)) {
		xbmc.VideoLibraryScan()
	} else {
		clearPageCache(ctx)
	}
...
func AddShow(ctx *gin.Context) {
	...
	if merge == "false" {
		if show, err := isDuplicateShow(tmdbId); err != nil {
			libraryLog.Warning(err)
			xbmc.Notify("Torrents", fmt.Sprintf("LOCALIZE[30287];;%s", show.Name), config.AddonIcon())
			return
		}
	} else {
	...
	if xbmc.DialogConfirm("Torrents", fmt.Sprintf("%s;;%s", label, show.Name)) {
		xbmc.VideoLibraryScan()
	} else {
		clearPageCache(ctx)
	}
...
func LibraryUpdate(db *bolt.DB) {
	...
	// Migrate old TorrentsDB.json
	if _, err := os.Stat(filepath.Join(libraryPath, "TorrentsDB.json")); err == nil {
		libraryLog.Warning("Found TorrentsDB.json, upgrading to BoltDB...")
		var oldDB struct {
			Movies []string `json:"movies"`
			Shows  []string `json:"shows"`
		}
		oldFile := filepath.Join(libraryPath, "TorrentsDB.json")
		...
		if err != nil {
			...
			if err := json.Unmarshal(file, &oldDB); err != nil {
				...
				os.Remove(oldFile)
				libraryLog.Notice("Successfully imported and removed TorrentsDB.json")
			}
		}
	}
	...
	go func() {
		...
		go func() {
			...
			if tmdb.WarmingUp == true {
				xbmc.Notify("Torrents", "LOCALIZE[30147]", config.AddonIcon())
			}
		}()
		started := time.Now()
		...
		if took.Seconds() > 30 {
			xbmc.Notify("Torrents", "LOCALIZE[30148]", config.AddonIcon())
		}
			...
			case <-time.After(3 * time.Second):
				...
				if len(episodes) > 1 {
					...
					if len(labels) > 0 {
						...
						if xbmc.DialogConfirm("Torrents", fmt.Sprintf("LOCALIZE[30278];;%s", label)) {
							xbmc.VideoLibraryClean()
						}
					}
				} else {
	...
	if updateDelay > 0 {
		if updateDelay < 10 {
			// Give time to Torrents to update its cache of libraryMovies, libraryShows and libraryEpisodes
			updateDelay = 10
		}
		go func() {
		...
		case <- markedForRemovalTicker.C:
			DB.View(func(tx *bolt.Tx) error {
				...
				b.ForEach(func(k, v []byte) error {
					...
					// Remove from Torrents's library to prevent duplicates
					if item.Type == "movie" {
						if _, err := isDuplicateMovie(strconv.Itoa(item.ID)); err != nil {
							...
							if err := removeMovie(nil, strconv.Itoa(item.ID)); err != nil {
								libraryLog.Warning("Nothing left to remove from Torrents")
							}
						}
					} else {
...
func UpdateLibrary(ctx *gin.Context) {
	...
	if xbmc.DialogConfirm("Torrents", "LOCALIZE[30288]") {
		xbmc.VideoLibraryScan()
	}
}
